...
def ReadText(fd):
    ...
    while qtydata > 0:
        ...
        response = response + chr(wiringpi.serialGetchar(fd))
        qtydata = qtydata - 1   
    return response
...
def RFIDSetup():
    ...
    if response == 0 and fd >0:
        ...
        print("PI setup complete on channel " + fd)
    else:
...
def FactoryReset(fd):
    ...
    wiringpi.serialPutchar(fd, 0xAA)
    time.sleep(0.1)
...
def SetPollingDalay(fd):
    ...
    wiringpi.serialPutchar(fd, 0x50)
    wiringpi.serialPutchar(fd, 0x00)
    # various polling delays possible, standard one uncommented
    #wiringpi.serialPutchar(fd, 0x00) # 0x00 is no delay
    #wiringpi.serialPutchar(fd, 0x20) # 0x20 is approx 20ms
    #wiringpi.serialPutchar(fd, 0x40) # 0x40 is approx 65ms
    wiringpi.serialPutchar(fd, 0x60) # 0x60 is approx 262ms
    #wiringpi.serialPutchar(fd, 0x80) # 0x60 is approx 1 Seconds
    #wiringpi.serialPutchar(fd, 0xA0) # 0x60 is approx 4 Seconds
    ...
    if ans == int("0xC0", 16):
        ...
        print("Unexpected response " + hex(ans))
...
def ReadTagPageZero(fd):
    ...
    while notag:
        ...
        wiringpi.serialPutchar(fd, 0x52)
        wiringpi.serialPutchar(fd, 0x00)
        time.sleep(0.1)
        ans = ReadInt(fd)
        # print "Tag Status: %s" % hex(ans) #Added for Debug purposes
...
def ReadTagAndBlocks(fd):
    ...
    while notag:
        ...
        wiringpi.serialPutchar(fd, 0x74)
        wiringpi.serialPutchar(fd, 0x04)
        time.sleep(0.1)
        ans = ReadInt(fd)
        # print "Tag Status: %s" % hex(ans) #Added for Debug purposes
        if ans == int("0xD6", 16):
...
def ChangeReaderOpMode(fd):
    ...
    while choice == "":
        ...
        if choice =="a" or choice == "A":
            desc = "Hitag H2"
            WaitForCTS()
            wiringpi.serialPutchar(fd, 0x76)
            wiringpi.serialPutchar(fd, 0x01) # 0x01 = H2
        elif choice =="b" or choice == "B":
            desc = "Hitag H1/S"
            WaitForCTS()
            wiringpi.serialPutchar(fd, 0x76)
            wiringpi.serialPutchar(fd, 0x02) # 0x01 = H1/S
        elif choice =="c" or choice == "C":
            desc = "Em / MC2000"
            WaitForCTS()
            wiringpi.serialPutchar(fd, 0x76)
            wiringpi.serialPutchar(fd, 0x03) # 0x03 = EM/MC2000
        else:
    ...
    if ans == int("0xC0", 16):
        # Positive result
        print("Reader Operating Mode " + desc)
    else:
        print("Unexpected response " + hex(ans))
        # clear the buffer
        wiringpi.serialFlush(fd)
